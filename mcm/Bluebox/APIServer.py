...
def stream_object(container_name, object_name):
    ...
    swift_object = swift.get_object_as_generator(container_name, object_name)
    swift_headers = swift_object[0]
    swift_data = swift_object[1]

    headers = {}
    headers["Content-Length"] = swift_headers.get("content-length")
    ...
    if "x-object-meta-content-type" in swift_headers:
        mime_type = swift_headers.get("x-object-meta-content-type")
    else:
        mime_type = swift_headers.get("content-type")

    return Response(swift_data, mimetype=mime_type, headers=headers)
